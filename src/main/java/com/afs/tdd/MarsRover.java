...
public class MarsRover {
    ...
    public void executeCommands(String commands){
        for (String singleCommand : commands.split("")) {
    ...
    private void move() {
        ...
        if (roverStatus.getDirection().equals("N")) roverStatus = new RoverStatus(locationX, locationY + 1, direction);
        if (roverStatus.getDirection().equals("S")) roverStatus = new RoverStatus(locationX, locationY - 1, direction);
        if (roverStatus.getDirection().equals("E")) roverStatus = new RoverStatus(locationX + 1, locationY, direction);
        if (roverStatus.getDirection().equals("W")) roverStatus = new RoverStatus(locationX - 1, locationY, direction);
